...
function commandRisks(hook: HookEntry): Diagnostic[] {
  const target = [hook.command, ...(hook.args ?? [])].filter(Boolean).join(" ");
  if (!target) return [];

  return scanExpression(target).map((pattern) =>
    diagnostic({
      code: "AGF502",
      ...
      data: { event: hook.event, risk: pattern.id, hookType: hook.type, analysis: "static-pattern-match" },
    }),
  );
}
...
function missingScripts(hook: HookEntry, files: readonly string[]): Diagnostic[] {
  ...
  let script = match[1] ?? match[2] ?? match[3];
