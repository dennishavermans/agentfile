...
describe("selectSources", () => {
  it("never carries a target's own files back at it", () => {
    ...
    expect(selectSources(configuration, "agents-md").always.map((entry) => entry.provenance.file)).toEqual([
      "CLAUDE.md",
    ]);
  });
  ...
  it("orders deterministically by file, then line", () => {
    const configuration = configurationWith({
      instructions: [instruction("b.md", "two"), instruction("a.md", "one")],
    });
...
describe("target compilers", () => {
  ...
  it("reports what the compiler itself does not carry, without blaming the target", () => {
    ...
    expect(agentsMd.diagnostics.some((entry) => entry.code === "AGF201" && entry.data?.feature === "skills")).toBe(
      true,
    );
  });
...
describe("planCompilation", () => {
  ...
  it("throws on an unknown target instead of guessing", () => {
    expect(() => planCompilation(configuration, { root: ROOT, fs: memoryFileSystem({}), targets: ["codex"] })).toThrow(
      /agents-md/,
    );
  });
